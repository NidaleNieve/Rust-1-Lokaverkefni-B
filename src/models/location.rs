...
impl FromStr for Location {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        if parts.len() != 2 {
            return Err("Vænt formað sem HA-123".into());
        }
        let house = parts[0].parse::<House>().map_err(|e: String| e)?;
        let digits = parts[1];
        if digits.len() < 2 {
            return Err("Eftir '-' þarf að vera a.m.k. tvær tölur".into());
        }
        let floor = digits.chars().next().unwrap();
        if !floor.is_ascii_digit() {
            return Err("Hæð þarf að vera tala (eitt staf).".into());
        }
        let floor = floor.to_digit(10).unwrap() as u8;
        let room_str = &digits[1..];
        let room: u16 = room_str
            .parse()
            .map_err(|_| "Herbergisnúmer ógilt".to_string())?;
